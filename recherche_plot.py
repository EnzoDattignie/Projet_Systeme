...
#Lists initialisation to store data
wavelength = []
intensity = []
...
if is_number(inf) and is_number(sup): #If those are numbers we change them into floats, otherwise we take the min and max from the list wavelength
    ...
    print('No value given as limit, plot on the whole sample')
...
#These lists will be the one plotted between the two wavelength limits given by the user
wavelengthf=[] 
intensityf=[]
...
plt.title("Graph showing intensity in function of the wavelength")
...
if len(wavelengthf)>0: #We check that lines aren't empty before ploting
    ...
    print('No data to graph on this interval')
