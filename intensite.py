...
if not(is_corrupted(sys.argv[0],sys.argv[0])):
    window = 10
    if len(sys.argv) > 2: #We check that a window is given and that it's a number, otherwise we let it at 10nm
    ...
    if (len(sys.argv) > 1) : #We verify that an argument is given, otherwise wwe can't open the file
        ...
        print("No file given, Please add a file as argument")

    #Print of every important value
    for key in dict :
